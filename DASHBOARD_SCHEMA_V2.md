...
## Экспортируемый Streamlit-проект

Скачанный `app.py` использует нативные компоненты Streamlit и Vega-Lite. Раскладка воспроизводится через адаптивную 12-колоночную CSS Grid-сетку с теми же размерами строк и отступов, что и холст конструктора.

- `layout.x` и `layout.y` сохраняют положение компонента на холсте;
- `layout.width` задаёт ширину в колонках, `layout.height` — целевую высоту карточки;
- фильтры, метрики и графики размещаются в одной сетке, а не отдельными блоками;
- график занимает всё доступное место внутри карточки и автоматически сокращает количество подписей осей;
- повторяющиеся значения X получают безопасные уникальные ключи, чтобы столбцы и точки не накладывались;
- для схем без `x/y` backend автоматически подбирает свободное место без пересечений.
- на узких экранах карточки переходят в вертикальный адаптивный список.

Интерактивные фильтры используют нативный `st.selectbox`: выбранное значение хранится в `st.session_state`, после чего метрики и графики пересчитываются на отфильтрованном наборе данных. Скачанный проект не зависит от `streamlit-elements`.
